        ...
        <main class="flex flex-1 flex-col">
            ...
            <section class="flex-1 px-4 py-6 lg:px-8 lg:py-8">
                <div class="mx-auto max-w-2xl">
                    <a href="{{ route('farm.reminders.show', [$farm, $reminder]) }}" class="inline-flex items-center gap-2 text-sm text-slate-500 transition hover:text-slate-700">
                    ...
                    <div class="mt-4 rounded-[2rem] border border-slate-200/60 bg-white p-6 shadow-sm shadow-slate-900/5 sm:p-8">
                        ...
                        <p class="mt-1 text-sm text-slate-500">Perbarui detail reminder. Target tidak dapat diubah — hapus dan buat ulang bila perlu.</p>

                        <form action="{{ route('farm.reminders.update', [$farm, $reminder]) }}" method="POST" class="mt-6 space-y-5">
                            @csrf
                            @method('PUT')

                            <div>
                                <label for="title" class="block text-sm font-semibold text-slate-700">Judul</label>
                                <input type="text" name="title" id="title" value="{{ old('title', $reminder->title) }}" required maxlength="255"
                            ...
                            <div>
                                ...
                                <textarea name="body" id="body" rows="3" required
                                    class="mt-1.5 block w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 transition focus:border-[#ffce54] focus:outline-none focus:ring-2 focus:ring-[#ffce54]/20">{{ old('body', $reminder->body) }}</textarea>
                                @error('body')
                                    <p class="mt-1.5 text-sm text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="datetime-local" name="starts_at" id="starts_at"
                                    value="{{ old('starts_at', $reminder->starts_at->format('Y-m-d\TH:i')) }}" required
                                    class="mt-1.5 block w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 transition focus:border-[#ffce54] focus:outline-none focus:ring-2 focus:ring-[#ffce54]/20">
                                @error('starts_at')
                                    <p class="mt-1.5 text-sm text-red-500">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <div class="mt-2 space-y-2">
                                    <label class="flex items-center gap-2 rounded-2xl border border-slate-200 px-4 py-2.5 text-sm">
                                        <input type="radio" name="recurrence[type]" value="none" @checked(old('recurrence.type', $reminder->recurrence['type'] ?? 'none') === 'none') class="accent-[#ffce54]">
                                        Sekali saja
                                    </label>
                                    <label class="flex items-center gap-2 rounded-2xl border border-slate-200 px-4 py-2.5 text-sm">
                                        <input type="radio" name="recurrence[type]" value="interval" @checked(old('recurrence.type', $reminder->recurrence['type'] ?? '') === 'interval') class="accent-[#ffce54]">
                                        Setiap beberapa hari
                                    </label>
                                    <label class="flex items-center gap-2 rounded-2xl border border-slate-200 px-4 py-2.5 text-sm">
                                        <input type="radio" name="recurrence[type]" value="weekly" @checked(old('recurrence.type', $reminder->recurrence['type'] ?? '') === 'weekly') class="accent-[#ffce54]">
                                        Setiap minggu di hari tertentu
                                    </label>
                                    <label class="flex items-center gap-2 rounded-2xl border border-slate-200 px-4 py-2.5 text-sm">
                                        <input type="radio" name="recurrence[type]" value="monthly" @checked(old('recurrence.type', $reminder->recurrence['type'] ?? '') === 'monthly') class="accent-[#ffce54]">
                                ...
                                <div class="mt-3">
                                    ...
                                    <input type="number" name="recurrence[every_days]" id="every_days" min="1"
                                        value="{{ old('recurrence.every_days', $reminder->recurrence['every_days'] ?? 1) }}"
                                        class="mt-1.5 block w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 transition focus:border-[#ffce54] focus:outline-none focus:ring-2 focus:ring-[#ffce54]/20">
                                </div>
                                        ...
                                        @foreach(['mon' => 'Senin', 'tue' => 'Selasa', 'wed' => 'Rabu', 'thu' => 'Kamis', 'fri' => 'Jumat', 'sat' => 'Sabtu', 'sun' => 'Minggu'] as $key => $label)
                                            <label class="flex items-center gap-1.5 rounded-xl bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-600">
                                                <input type="checkbox" name="recurrence[days_of_week][]" value="{{ $key }}"
                                                    @checked(in_array($key, old('recurrence.days_of_week', $reminder->recurrence['days_of_week'] ?? []), true))
                                                    class="accent-[#ffce54]">
                                                {{ $label }}
                                            </label>
                                        @endforeach
                                        ...
                                        @for($day = 1; $day <= 31; $day++)
                                            <label class="flex items-center justify-center rounded-xl bg-slate-100 py-1.5 text-xs font-semibold text-slate-600">
                                                <input type="checkbox" name="recurrence[days_of_month][]" value="{{ $day }}"
                                                    @checked(in_array($day, old('recurrence.days_of_month', $reminder->recurrence['days_of_month'] ?? []), true))
                                                    class="accent-[#ffce54]">
                            ...
                            <div>
                                ...
                                <select name="advance_notify_minutes" id="advance_notify_minutes"
                                    ...
                                    <option value="30" @selected(old('advance_notify_minutes', $reminder->advance_notify_minutes) == 30)>30 menit sebelumnya</option>
                                    <option value="60" @selected(old('advance_notify_minutes', $reminder->advance_notify_minutes) == 60)>1 jam sebelumnya</option>
                                    <option value="1440" @selected(old('advance_notify_minutes', $reminder->advance_notify_minutes) == 1440)>1 hari sebelumnya</option>
                                </select>
