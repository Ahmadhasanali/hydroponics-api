...
@section('title', 'Laporan Penggunaan Nutrisi')

@section('content')
    <div>
        <h2 class="text-lg font-semibold text-slate-900">Laporan Penggunaan Nutrisi</h2>
        <p class="mt-1 text-sm text-slate-500">Filter dan lihat total penggunaan Nutrisi A & B</p>
    </div>

    <div class="mt-4 rounded-[2rem] border border-slate-200/60 bg-white p-6 shadow-sm shadow-slate-900/5 sm:p-8">
        <form method="GET" action="{{ route('staff.reports.nutrient') }}" class="flex flex-wrap items-end gap-4">
            <div class="flex-1 min-w-[200px]">
                ...
                <select name="tank_id" id="tank_id" required
                    ...
                    @foreach($tanks as $tank)
                        <option value="{{ $tank->id }}" {{ request('tank_id') == $tank->id ? 'selected' : '' }}>{{ $tank->name }}</option>
                    @endforeach
                </select>
            </div>
    ...
    @if($aggregates)
        <div class="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            <div class="rounded-[2rem] border border-slate-200/60 bg-white p-5 shadow-sm shadow-slate-900/5">
                <p class="text-xs font-semibold uppercase tracking-wider text-slate-500">Total Nutrisi A</p>
                <p class="mt-2 text-2xl font-bold text-sky-600">{{ number_format($aggregates->total_nutrient_a_ml, 2) }} ml</p>
            </div>

            <div class="rounded-[2rem] border border-slate-200/60 bg-white p-5 shadow-sm shadow-slate-900/5">
                <p class="text-xs font-semibold uppercase tracking-wider text-slate-500">Total Nutrisi B</p>
                <p class="mt-2 text-2xl font-bold text-sky-600">{{ number_format($aggregates->total_nutrient_b_ml, 2) }} ml</p>
            </div>
        </div>
    @elseif(request()->has('tank_id'))
        <div class="mt-8 flex flex-col items-center rounded-[2rem] border-2 border-dashed border-slate-300 bg-white px-6 py-16 text-center">
            ...
            <p class="mt-1 text-sm text-slate-500">Tidak ada data penambahan nutrisi untuk filter yang dipilih.</p>
        </div>
    @endif
