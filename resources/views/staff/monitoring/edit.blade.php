...
@section('content')
    <div class="mx-auto max-w-2xl">
        <a href="{{ route('staff.monitoring.index') }}" class="inline-flex items-center gap-2 text-sm text-slate-500 transition hover:text-slate-700">
        ...
        <div class="mt-4 rounded-[2rem] border border-slate-200/60 bg-white p-6 shadow-sm shadow-slate-900/5 sm:p-8">
            ...
            <p class="mt-1 text-sm text-slate-500">Perbarui PPM, pH, dan suhu air.</p>

            <form action="{{ route('staff.monitoring.update', $dailyMonitoring) }}" method="POST" class="mt-6 space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="tank_id" id="tank_id" required
                        ...
                        @foreach($tanks as $tank)
                            <option value="{{ $tank->id }}" @selected(old('tank_id', $dailyMonitoring->tank_id) == $tank->id)>{{ $tank->name }} ({{ number_format($tank->capacity_liter, 0) }} L)</option>
                        @endforeach
                    </select>
                    @error('tank_id')
                ...
                <div class="flex items-center gap-3 pt-2">
                    ...
                    <a href="{{ route('staff.monitoring.index') }}"
