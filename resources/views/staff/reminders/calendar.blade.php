...
@section('content')
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('staff.reminders.calendar', ['month' => $start->copy()->subMonth()->format('Y-m')]) }}"
            ...
            <a href="{{ route('staff.reminders.calendar', ['month' => $start->copy()->addMonth()->format('Y-m')]) }}"
                class="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-600 transition hover:bg-slate-50">
                ...
                <div class="min-h-24 border-b border-r border-slate-100 p-2 {{ $dayReminders->isNotEmpty() ? 'bg-[#ffce54]/5' : '' }}">
                    ...
                    <div class="mt-1.5 space-y-1">
                        @foreach($dayReminders->take(3) as $item)
                            <span class="block truncate rounded-lg bg-slate-100 px-2 py-1 text-xs font-medium text-slate-700">
                                {{ $item->reminder->title }}
                            </span>
                        @endforeach
