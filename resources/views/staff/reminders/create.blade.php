...
@section('content')
    <div class="mx-auto max-w-2xl">
        <a href="{{ route('staff.reminders.index') }}" class="inline-flex items-center gap-2 text-sm text-slate-500 transition hover:text-slate-700">
        ...
        <div class="mt-4 rounded-[2rem] border border-slate-200/60 bg-white p-6 shadow-sm shadow-slate-900/5 sm:p-8">
            ...
            <p class="mt-1 text-sm text-slate-500">Buat pengingat jadwal untuk dirimu atau petugas lain di kebun.</p>

            <form action="{{ route('staff.reminders.store') }}" method="POST" class="mt-6 space-y-5">
                @csrf
                ...
                <div>
                    ...
                    <textarea name="body" id="body" rows="3" required
                        class="mt-1.5 block w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 transition focus:border-[#ffce54] focus:outline-none focus:ring-2 focus:ring-[#ffce54]/20"
                        placeholder="Deskripsi reminder">{{ old('body') }}</textarea>
                    @error('body')
                        <p class="mt-1.5 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center gap-3 pt-2">
                    ...
                    <a href="{{ route('staff.reminders.index') }}"
