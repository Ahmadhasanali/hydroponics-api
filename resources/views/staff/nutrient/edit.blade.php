...
@section('title', 'Edit Data AB Mix')

@section('content')
    <div class="mx-auto max-w-2xl">
        <a href="{{ route('staff.nutrient.index') }}" class="inline-flex items-center gap-2 text-sm text-slate-500 transition hover:text-slate-700">
            <i class="bi bi-arrow-left"></i>
            Kembali
        </a>

        <div class="mt-4 rounded-[2rem] border border-slate-200/60 bg-white p-6 shadow-sm shadow-slate-900/5 sm:p-8">
            <h2 class="text-lg font-semibold text-slate-900">Edit Penambahan Nutrisi (AB Mix)</h2>
            <p class="mt-1 text-sm text-slate-500">Perbarui PPM sebelum & sesudah beserta dosis A/B.</p>

            <form action="{{ route('staff.nutrient.update', $nutrientAddition) }}" method="POST" class="mt-6 space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="date" name="log_date" id="log_date" value="{{ old('log_date', $nutrientAddition->log_date?->format('Y-m-d')) }}" required
                        class="mt-1.5 block w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 transition focus:border-[#ffce54] focus:outline-none focus:ring-2 focus:ring-[#ffce54]/20">
                    @error('log_date')
                        <p class="mt-1.5 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="number" name="ppm_before" id="ppm_before" step="0.01" min="0" max="3000" value="{{ old('ppm_before', $nutrientAddition->ppm_before) }}" required
                    ...
                    <div>
                        ...
                        <input type="number" name="ppm_after" id="ppm_after" step="0.01" min="0" max="3000" value="{{ old('ppm_after', $nutrientAddition->ppm_after) }}" required
                    ...
                    <div>
                        ...
                        <input type="number" name="nutrient_a_ml" id="nutrient_a_ml" step="0.01" min="0" max="10000" value="{{ old('nutrient_a_ml', $nutrientAddition->nutrient_a_ml) }}" required
                    ...
                    <div>
                        ...
                        <input type="number" name="nutrient_b_ml" id="nutrient_b_ml" step="0.01" min="0" max="10000" value="{{ old('nutrient_b_ml', $nutrientAddition->nutrient_b_ml) }}" required
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3"
                        class="mt-1.5 block w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 transition focus:border-[#ffce54] focus:outline-none focus:ring-2 focus:ring-[#ffce54]/20"
                        placeholder="Opsional">{{ old('notes', $nutrientAddition->notes) }}</textarea>
                </div>
                ...
                <div class="flex items-center gap-3 pt-2">
                    ...
                    <a href="{{ route('staff.nutrient.index') }}"
