...
@section('title', 'Catatan AB Mix Saya')

@section('content')
    <div>
        <h2 class="text-lg font-semibold text-slate-900">Catatan AB Mix Saya</h2>
        <p class="mt-1 text-sm text-slate-500">Riwayat penambahan nutrisi yang Anda catat.</p>
    </div>
    ...
    @if($additions->isEmpty())
        <div class="mt-8 flex flex-col items-center rounded-[2rem] border-2 border-dashed border-slate-300 bg-white px-6 py-16 text-center">
            ...
            <a href="{{ route('staff.nutrient.create') }}" class="mt-5 inline-flex items-center gap-2 rounded-2xl bg-[#ffce54] px-5 py-2.5 text-sm font-bold text-[#1a1c1e] transition hover:bg-[#f0b830]">
                        ...
                        <tr class="transition hover:bg-slate-50/50">
                            ...
                            <td class="px-5 py-3">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('staff.nutrient.edit', $a) }}"
                                    ...
                                    <form action="{{ route('staff.nutrient.destroy', $a) }}" method="POST" onsubmit="return confirm('Hapus catatan ini?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
