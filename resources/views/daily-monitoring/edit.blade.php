...
@section('title', 'Edit Daily Monitoring')
            ...
            <section class="flex-1 px-4 py-6 lg:px-8 lg:py-8">
                <div class="mx-auto max-w-2xl">
                    ...
                    <div class="mt-4 rounded-[2rem] border border-slate-200/60 bg-white p-6 shadow-sm shadow-slate-900/5 sm:p-8">
                        <h2 class="text-lg font-semibold text-slate-900">Edit Daily Monitoring</h2>
                        <p class="mt-1 text-sm text-slate-500">Perbarui data monitoring harian.</p>

                        <form action="{{ route('daily-monitoring.update', $dailyMonitoring) }}" method="POST" class="mt-6 space-y-5" id="monitoring-form">
                            @csrf
                            @method('PUT')
                            ...
                            <div>
                                ...
                                <select name="tank_id" id="tank_id" required
                                    class="mt-1.5 block w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 transition focus:border-[#ffce54] focus:outline-none focus:ring-2 focus:ring-[#ffce54]/20 @error('tank_id') border-red-300 @enderror">
                                    ...
                                    @foreach($tanks as $tank)
                                        <option value="{{ $tank->id }}"
                                            ...
                                            @selected(old('tank_id', $dailyMonitoring->tank_id) == $tank->id)>{{ $tank->name }} ({{ number_format($tank->capacity_liter, 0) }} L)</option>
                                    @endforeach
                                </select>
                                @error('tank_id')
                            ...
                            <div>
                                ...
                                <input type="date" name="log_date" id="log_date" value="{{ old('log_date', $dailyMonitoring->log_date->format('Y-m-d')) }}" required
                            ...
                            <div class="grid gap-4 sm:grid-cols-3">
                                <div>
                                    ...
                                    <input type="number" name="ppm" id="ppm" step="0.01" min="0" max="3000" value="{{ old('ppm', $dailyMonitoring->ppm) }}" required
                                ...
                                <div>
                                    ...
                                    <input type="number" name="ph" id="ph" step="0.01" min="0" max="14" value="{{ old('ph', $dailyMonitoring->ph) }}" required
                                ...
                                <div>
                                    ...
                                    <input type="number" name="water_temperature" id="water_temperature" step="0.01" value="{{ old('water_temperature', $dailyMonitoring->water_temperature) }}"
                            ...
                            <div>
                                ...
                                <textarea name="notes" id="notes" rows="3"
                                    ...
                                    placeholder="Opsional">{{ old('notes', $dailyMonitoring->notes) }}</textarea>
                                @error('notes')
                                    <p class="mt-1.5 text-sm text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
