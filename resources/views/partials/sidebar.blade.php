...
{{-- Sidebar --}}
<aside id="sidebar"
    class="fixed inset-y-0 left-0 z-30 w-72 -translate-x-full transform overflow-y-auto bg-white px-4 py-6 shadow-2xl transition-all duration-300 ease-in-out
    ...
    <div class="hidden lg:flex lg:items-center lg:gap-3 lg:shrink-0">
        ...
        <div id="sidebarLogoText" class="sidebar-text space-y-0.5 overflow-hidden">
            <p class="text-base font-semibold text-slate-900 whitespace-nowrap">Hydro Monitor</p>
            <p class="text-xs leading-5 text-slate-500 whitespace-nowrap">Pantau kondisi &amp; nutrisi</p>
        </div>
    </div>

    {{-- Nav --}}
    <nav class="mt-6 space-y-1 text-sm font-medium text-slate-700 flex-1">
        <a href="{{ route('dashboard') }}"
    ...
    <div id="sidebarStatusWidget" class="sidebar-text mt-8 rounded-[2rem] border border-slate-200/80 bg-slate-50 p-4 text-sm text-slate-700 overflow-hidden">
        <p class="font-semibold text-slate-900 whitespace-nowrap">Status Sistem</p>
        <p class="mt-2 text-slate-500 text-xs leading-5">Semua sensor terhubung, data rutin tersedia setiap 15 menit.</p>
    </div>
</aside>
